...
COMMAND=disable_win_key/disable_win_key
...
while test $# -gt 0
	...
	if [ $VALID == false ]
		...
		if [ $USER != "lab" ]
			...
			echo '**********************'
			#thank you 1: http://support.microsoft.com/kb/216893
			#thank you 2: http://stackoverflow.com/questions/13809856/run-reg-command-in-cmd-bat-file
			pssh -h clients_${GRP}.txt -o out_dir -e err_dir 'runas /user:administrator /savecred "regedit.exe /S C:\\Users\\lab\\Downloads\\scripts\\disable_win_key\\KillWinKey.reg"'
			echo '#############'
