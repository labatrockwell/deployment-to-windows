...
while test $# -gt 0
	...
	if [ $VALID == false ]
		...
		if [ $USER != "lab" ]
			...
			echo "##################"
			#pscp -h clients_${GRP}.txt -o out_dir -e err_dir -r test_ac /cygdrive/c/Users/lab/Downloads/scripts/
			echo "##################"
			echo "#### run powershell & report"
			echo "##################"
			pssh -h clients_${GRP}.txt -o out_dir -e err_dir -i "powershell C:\\\\Users\\\\lab\\\\Downloads\\\\scripts\\\\test_ac\\\\test_ac.ps1"
		fi
		###############
		###############
	fi
